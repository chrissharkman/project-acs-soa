...
import ch.comem.acs.transport.CustomerTrans;
import ch.comem.acs.transport.StatusTrans;
import ch.comem.acs.transport.VehicleTypeTrans;
...
/**
 * This Class contains all the possible operations of the web service. Actually,
 * the service proposes the following main functions:
...
public class ACSInsuranceWS {
     ...
     *
     * @param certificateT the certificateTrans object, which will be saved via
     * certificate object in the bd.
    ...
    /**
     * Web service operation to modify a certificate. This way a certificate can
     * be changed on multiple fields.
     *
     * @param certificateId the id of the certificate to change.
     * @param vin the vehicule identification number, a string. if null, vin is
     * not modified.
     ...
     * @return 1 if updates was successful, 0 is nothing was modified, -1 if an
     * error occured (Rollback included).
     */
    ...
    /**
     * Get a specific customer in BD
     *
     * @param idiustomer id of specific customer
     * @return a customerTrans
     */
    @WebMethod(operationName = "getCustomer")
    public CustomerTrans getCustomer(@WebParam(name = "iustomerId") int idiustomer) {

        CustomerTrans it = new CustomerTrans();
        Customer i = ControllerACS.getCustomer(idiustomer);
        it = Converter.customerToCustomerTrans(i);
        return it;
    }

    /**
     * This function get all type of vehicle in DB
     *
    ...
    public List<VehicleTypeTrans> getVehicleType() {
        ...
        List<VehicleType> listeV = ControllerACS.getVehicleType();
        for (VehicleType v : listeV) {
            listVt.add(Converter.vehicleTypeToVehicleTypeTrans(v));
        }
        return listVt;
    }
